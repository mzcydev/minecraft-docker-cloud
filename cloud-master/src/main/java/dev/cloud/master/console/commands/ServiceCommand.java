...
public class ServiceCommand implements Command {
    ...
    @Override public String getName()        { return "service"; }
    @Override public String getDescription() { return "Manage running services."; }
    @Override public String getUsage()       { return "service <list|stop|info> [name]"; }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "list" -> {
                ...
                if (services.isEmpty()) { sender.sendMessage("No services running."); return; }
            ...
            case "stop" -> {
                if (args.length < 2) { sender.sendMessage("Usage: service stop <name>"); return; }
            ...
            case "info" -> {
                if (args.length < 2) { sender.sendMessage("Usage: service info <name>"); return; }
                serviceRegistry.findByName(args[1]).ifPresentOrElse(s -> {
